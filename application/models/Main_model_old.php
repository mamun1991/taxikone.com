...
// $this->db->order_by("display_order", "asc");
	 $query=$this->db->query(" SELECT * FROM hotel LEFT JOIN order_by_hotel ON hotel.id=order_by_hotel.hotel_id ORDER BY show_seq DESC");
	 // var_dump($query->result());die;
// $query = $this->db->get(); 
//  return $query->result();

 // $query =$this->db->select('*')->from('hotel')->join('order_by_hotel','hotel.id = order_by_hotel.hotel_id','full')->order_by("show_seq", "DESC")->get();
...
public function driver_detials(){
	...
	// 			->get('driver')->result();
	$query=$this->db->query("SELECT driver.drive_name, driver.id FROM driver INNER JOIN order_by_driver ON driver.id=order_by_driver.driver_id ORDER BY show_seq DESC");
return $query->result();
...
public function fetch_appoint_details_by_user($id){
